...
export function initScene(canvasID) {
  ...
  // xz is front 
  // yz is right 
  const planeMaterial = new THREE.MeshPhongMaterial( {color: 0xdddddd, side: THREE.DoubleSide, depthWrite: false, transparent: true, opacity:0.03} );
  ...
  const dotMaterial = new THREE.PointsMaterial({ size: 5, color: bluetiful, sizeAttenuation: false });
